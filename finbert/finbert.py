...
def predict(text, model, date, year, week, write_to_csv=False, path=None):
    """
    ...
    result = pd.DataFrame(columns=['sentence', 'date', 'year', 'week', 'logit','prediction','sentiment_score'])
    ...
    if write_to_csv:
        result.to_csv(path,sep=',', mode='a', index=False, header=False)
